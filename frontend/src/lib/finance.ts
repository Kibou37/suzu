...
/** Classic annuity monthly payment */
...
export function buildFinanceQuote(
  ...
  const downPayment = Math.min(price, Math.max(0, Number(input.downPayment) || 0));
  const termMonths = Math.min(
    rates.maxTermMonths,
    Math.max(rates.minTermMonths, Math.round(Number(input.termMonths) || rates.minTermMonths)),
  );
  ...
  const monthlyPayment = annuityPayment(financedAmount, annualRatePercent, termMonths);
  ...
  for (let month = 1; month <= termMonths; month += 1) {
    ...
    schedule.push({
      month,
      payment: month === termMonths ? round2(principal + interest) : monthlyPayment,
      principal,
      interest,
      balance,
    });
